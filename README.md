...
cd SlideNode
make setup    # 一次性初始化（创建 venv、安装依赖、复制 .env）
make dev      # 启动 backend(:8000) + frontend(:5173)，Ctrl+C 同时停止
```
...
make local-up-quick  # 不重新构建（纯代码改动时更快）
...
│   └── src/
│       ├── components/
│       │   ├── editor/   # 编辑器组件 (双栏、工具栏、引文面板)
│       │   ├── layout/   # 布局组件 (AppShell、侧边栏)
│       │   ├── main/     # 主区域状态 (空态、处理中)
│       │   ├── shared/   # 共享组件 (错误提示、确认框)
│       │   ├── ui/       # shadcn/ui 基础组件
│       │   └── upload/   # 上传组件 (对话框、拖放区)
│       ├── hooks/        # 自定义 Hooks (状态、轮询、草稿)
│       ├── lib/          # 工具函数
│       └── types/        # TypeScript 类型
├── infra/               # 基础设施
│   ├── docker-compose.yml
...
# 运行所有测试
make dev-test
# 或: cd backend && PYTHONPATH=. .venv/bin/python3.12 -m pytest tests/ -v

# 运行单个测试
cd backend && PYTHONPATH=. .venv/bin/python3.12 -m pytest tests/test_health.py -v
```
